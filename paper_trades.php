...
require_once __DIR__.'/bin/paper/TradeReview.php';
require_once __DIR__.'/bin/paper/Chrome.php';
use ChartEntryLab\PaperJournal;
function th(mixed $x): string {return paper_esc($x);}
...
<p class="paper-note">R = 기록된 순손익 ÷ 주문 당시 계획 위험금액. 청산 사유와 패턴 집계에는 같은 거래가 각각 포함되므로 두 표본을 합산하지 않습니다.</p>
</section>
<section class="panel">
<h2 class="paper-section-title">거래별 내역</h2>
...
<?php if($t['post_stop']): ?><h3 class="paper-section-title">손절 이후 흐름</h3><p>손절 다음 거래봉부터 계산합니다. 손절 당일 반등은 제외합니다. 진입가 회복은 해당 N번째 봉의 종가 기준, 비용 제외입니다. 손절하지 않고 보유했을 때의 수익률을 뜻하지 않습니다.</p>
<div class="scan-table-wrap"><table class="scan-table"><thead><tr><th>기간</th><th>상태</th><th>N봉째 종가 / 손절 체결가</th><th>N봉째 종가 진입가 회복</th><th>기간 고점 / 저점 (손절 체결가 대비)</th></tr></thead><tbody>
<?php foreach($t['post_stop'] as $n=>$p): ?><tr><td><?= th($n) ?>거래봉</td><td><?= th(tl($p['status']).' ('.$p['observed'].'봉)') ?></td><td class="mono"><?= th(tn($p['close_vs_exit_pct'])) ?>%</td><td><?= $p['recovered_entry_at_close']===null?'—':($p['recovered_entry_at_close']?'예':'아니오') ?></td><td class="mono"><?= th(tn($p['max_high_vs_exit_pct']).'% / '.tn($p['min_low_vs_exit_pct'])) ?>%</td></tr><?php endforeach ?></tbody></table></div><?php endif ?>
</details>
<?php endforeach ?>
<p class="paper-note">거래일은 계좌에 관측된 평가 세션 기준입니다. 거래소 휴일·거래정지 전체 달력을 검증하는 기능은 아니며, 관측된 품질 차단/누락을 건너뛰어 계산하지 않습니다. 보유 중 거래는 완료 거래 집계에서 제외합니다.</p>
</section>
<?php endif;
paper_close();
