...
?>
<form class="paper-filter"><?php paper_experiment_field($id); ?><label>기록 종류 <select name="mode"><option value="forward" <?= $mode==='forward'?'selected':'' ?>>앞으로 기록</option><option value="replay" <?= $mode==='replay'?'selected':'' ?>>과거 재현</option></select></label><button>조회</button></form>
...
<p class="paper-note">동일 전략 확인: <?= eh(paper_ko((string) $r['identity_check'])) ?> · 판단 보류 사유: <?= eh(paper_ko_join($r['reasons'])) ?></p>
...
<p class="paper-note">통화 <?= eh(paper_ko((string) $r['arms']['baseline']['currency'])) ?>. 자금 사용률은 종가 보유평가액과 미체결 예약금의 합을 평가자산으로 나눈 세션별 단순 평균입니다. 낙폭은 유효 종가 기준이며 장중 낙폭이 아닙니다.</p>
