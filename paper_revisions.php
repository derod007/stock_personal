...
?>
<form class="paper-filter"><label>계좌 ID <input name="account" value="<?= rh($id) ?>"></label><select name="mode"><option value="forward" <?= $mode==='forward'?'selected':'' ?>>앞으로 기록</option><option value="replay" <?= $mode==='replay'?'selected':'' ?>>과거 재현</option></select><button>조회</button></form>
...
<?php else: ?>
<section class="panel">
<p class="paper-lede">발견 시각 (UTC): <?= rh(gmdate('Y-m-d H:i:s',$report['detected_at'])) ?> · 변경 봉 <?= rh($report['change_count']) ?>개 · <?= rh($report['classification']) ?></p>
...
<tr><td class="mono"><?= rh($row['symbol'].' / '.gmdate('Y-m-d H:i',$row['session'])) ?></td><td><?= rh($row['kind'].' / '.$f['field'].($f['representation_only']?' (숫자 값 동일·표현 변경)':'')) ?></td><td><?= rh($f['old_present']?rv($f['old']):'필드 없음') ?></td><td><?= rh($f['new_present']?rv($f['new']):'필드 없음') ?></td><td class="mono"><?= rh(rv($f['delta']).' / '.($f['change_pct']===null?'—':round($f['change_pct'],6).'%')) ?></td></tr>
