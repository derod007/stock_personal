...
require __DIR__.'/bin/bootstrap.php';require_once __DIR__.'/bin/paper/Universe.php';require_once __DIR__.'/bin/paper/Chrome.php';
...
<form class="paper-filter"><?php paper_account_field($id, '연구 계좌', ['research-kr-v1', 'research-us-v1']); ?><label>기록 종류 <select name="mode"><option value="forward" <?= $mode==='forward'?'selected':'' ?>>앞으로 기록</option><option value="replay" <?= $mode==='replay'?'selected':'' ?>>과거 재현</option></select></label><button>조회</button></form>
...
<p class="paper-lede"><?= uh(paper_account_name($id)) ?> · 종목군 정의일 <?= uh($config['universe']['defined_on']??'—') ?> · 통화 <?= uh(paper_ko((string) $config['currency'])) ?> · 실제 기록 시작 <?= uh(ut($r['started_at']??null)) ?></p>
<details class="paper-trade" open><summary>고정 종목 목록</summary><div class="scan-table-wrap"><table class="scan-table"><thead><tr><th>종목 코드</th><th>연구 업종</th></tr></thead><tbody><?php foreach($config['symbols'] as $sym=>$sector): ?><tr><td><?= uh($sym) ?></td><td><?= uh(paper_ko((string) $sector)) ?></td></tr><?php endforeach ?></tbody></table></div></details>
...
<p class="paper-lede">시작 <?= uh(ut($t['started_at'])) ?> · 결과 <?= uh(paper_ko((string) $t['status'])) ?> · <?= uh(paper_ko((string) ($t['stage']??'—'))) ?> · 오류 <?= uh(paper_ko((string) ($t['error_type']??'없음'))) ?></p>
...
<div class="scan-table-wrap"><table class="scan-table"><thead><tr><th>종목</th><th>수집 상태</th><th>유효 봉</th><th>사유</th></tr></thead><tbody><?php foreach($t['coverage']['symbols'] as $symbol=>$v): ?><tr><td><?= uh($symbol) ?></td><td><?= uh(paper_ko((string) $v['status'])) ?></td><td><?= uh($v['valid_bars']??0) ?></td><td><?= uh(paper_ko((string) ($v['reason']??'—'))) ?></td></tr><?php endforeach ?></tbody></table></div>
...
<?php if($r): ?>
<h2>전체 기록 성과</h2><p class="paper-lede">마지막 평가 <?= uh(ut($r['last_session'])) ?> · <?= $r['halted']?'계좌 중단':'계좌 중단 표시 없음' ?> · 완료 <?= uh($r['closed']) ?>건 · 실현손익 <?= uh(number_format($r['realized'],2)) ?> <?= uh(paper_ko((string) $r['config']['currency'])) ?></p>
<?php if($r['issues']): ?><p class="paper-alert">계좌 합계 확인 필요: <?= uh(paper_ko_join($r['issues'])) ?></p><?php endif ?>
...
<?php foreach(['sectors'=>'업종별','symbols'=>'종목별'] as $key=>$title): ?><h3><?= uh($title) ?></h3><div class="scan-table-wrap"><table class="scan-table"><thead><tr><th>구분</th><th>평가 / 조건 충족</th><th>주문 / 체결</th><th>청산 / 이익</th><th>실현손익</th></tr></thead><tbody><?php foreach($r[$key] as $v): ?><tr><td><?= uh(paper_ko((string) ($v['symbol']??$v['sector']))) ?></td><td><?= uh($v['evaluated'].' / '.$v['ready']) ?></td><td><?= uh($v['orders'].' / '.$v['fills']) ?></td><td><?= uh($v['closed'].' / '.$v['wins']) ?></td><td><?= uh(number_format($v['net_pnl'],2)) ?></td></tr><?php endforeach ?></tbody></table></div><?php endforeach ?>
<h3>종목별 관측·제외 사유</h3><?php foreach($r['symbols'] as $v): $snap=$v['last_snapshot']; ?><details class="paper-trade"><summary><?= uh($v['symbol']) ?> · 최신 품질 <?= uh($snap?paper_ko((string) $snap['quality']):'기록 없음') ?></summary><p>기록 종류 <?= uh(paper_ko_counts($v['origins'])) ?> · 제외 <?= uh($v['excluded']?paper_ko_counts($v['excluded']):'없음') ?><?php if($snap): ?> · 품질 사유 <?= uh(paper_ko_join($snap['reasons']??[])) ?><?php endif ?></p></details><?php endforeach ?>
