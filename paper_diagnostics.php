...
?>
<form class="paper-filter"><label>계좌 ID <input name="account" value="<?= dh($id) ?>"></label><select name="mode"><option value="forward" <?= $mode==='forward'?'selected':'' ?>>앞으로 기록</option><option value="replay" <?= $mode==='replay'?'selected':'' ?>>과거 재현</option></select><button>조회</button></form>
<section class="panel">
...
<p class="paper-lede">최근 시작 <?= dh(dt($r['started_at'])) ?> KST · <?= dh(PaperDiagnostics::label($r['outcome'])) ?> · 단계 <?= dh($r['stage']) ?></p>
...
<?php foreach($runs['recent'] as $r): ?><tr><td class="mono"><?= dh(dt($r['started_at']).' / '.dt($r['finished_at']??0)) ?></td><td><?= dh(PaperDiagnostics::label($r['outcome'])) ?></td><td><?= dh($r['stage'].' / '.($r['error_type']??'—')) ?></td><td class="mono"><?= dh(dt($r['summary']['last_session']??0)) ?></td></tr><?php endforeach ?></tbody></table></div>
<?php endif ?>
...
<?php if(!empty($d['state']['halted'])): ?><p class="paper-alert">계좌 중단: <?= dh($d['state']['halt_reason']??'unknown') ?>. 정상적인 추천 없음과 다릅니다.</p><?php endif ?>
