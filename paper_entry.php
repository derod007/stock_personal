...
$id=$_GET['account']??'research-us-v1';$mode=$_GET['mode']??'forward';$origin=$_GET['origin']??($mode==='replay'?'replay':'forward');$exp=$_GET['experiment']??(str_contains((string)$id,'kr')?'kr-volume95-v1':'us-volume95-v1');
...
?>
<form class="paper-filter"><?php paper_account_field($id, '원본 계좌'); ?><?php paper_experiment_field($exp); ?><label>모드 <select name="mode"><?php foreach(['forward'=>'당시 기록','replay'=>'과거 재현'] as $x=>$label): ?><option value="<?= paper_esc($x) ?>" <?= $mode===$x?'selected':'' ?>><?= paper_esc($label) ?></option><?php endforeach ?></select></label><label>관측 종류 <select name="origin"><?php foreach(['forward'=>'당시 기록','catchup'=>'사후 기록','replay'=>'과거 재현'] as $x=>$label): ?><option value="<?= paper_esc($x) ?>" <?= $origin===$x?'selected':'' ?>><?= paper_esc($label) ?></option><?php endforeach ?></select></label><button>조회</button></form>
...
<div class="scan-table-wrap"><table class="scan-table"><thead><tr><th>조건</th><th>통과</th><th>탈락</th><th>미평가</th><th>단독 탈락</th></tr></thead><tbody>
<?php $labels=['rising_structure'=>'상승 구조','atr_valid'=>'ATR 유효','valid_zone'=>'유효 가격 구간','near_ma20'=>'20일선 눌림','volume_contracted'=>'눌림 거래량 감소','recovery_close'=>'직전 고가 위 종가','bullish_candle'=>'양봉','volume_recovery'=>'거래량 회복','fresh_confirmation'=>'새 확인 신호'];foreach($r['gates'] as $k=>$row): ?><tr><th><?= paper_esc($labels[$k]??$k) ?></th><?php foreach($row as $n): ?><td><?= $n ?></td><?php endforeach ?></tr><?php endforeach ?></tbody></table></div>
<?php foreach(['statuses'=>'최종 신호 상태','pattern_statuses'=>'패턴별 상태','decisions'=>'주문 차단 사유','cancellations'=>'미체결·취소 사유'] as $key=>$label): ?><h3 class="paper-section-title"><?= $label ?></h3><ul class="paper-note"><?php foreach($r[$key] as $reason=>$n): ?><li><?= paper_esc(paper_ko((string) $reason)) ?>: <?= $n ?></li><?php endforeach ?></ul><?php endforeach ?>
...
<?php else: ?>
<p><?= paper_esc($health) ?></p><p><?= paper_esc($pair['decision']) ?> · <?= paper_esc(paper_ko_join($pair['reasons'])) ?></p>
...
<div class="scan-table-wrap"><table class="scan-table"><thead><tr><th>항목</th><th>기준</th><th>완화 후보</th></tr></thead><tbody>
<?php foreach(['ready'=>'진입 확인','orders'=>'주문','fills'=>'체결'] as $k=>$label): ?><tr><th><?= $label ?> (<?= paper_esc(paper_ko((string) $origin)) ?>)</th><?php foreach(['baseline','candidate'] as $arm): ?><td><?= $arms[$arm][$k] ?></td><?php endforeach ?></tr><?php endforeach ?>
<?php foreach(['net_pnl'=>'비용·평가손익 포함 순손익','closed_trades'=>'완료 거래','max_drawdown_pct'=>'최대 종가 낙폭 %','return_pct'=>'계좌 수익률 %'] as $k=>$label): ?><tr><th><?= $label ?> (전체 기간)</th><?php foreach(['baseline','candidate'] as $arm): ?><td><?= paper_esc(round($pair['arms'][$arm][$k],3)) ?></td><?php endforeach ?></tr><?php endforeach ?></tbody></table></div>
<p>통화 <?= paper_esc(paper_ko((string) $pair['arms']['baseline']['currency'])) ?>. 양쪽은 같은 날 빈 계좌로 시작합니다. 기존 원본 계좌 누적 성과와 직접 비교하지 않습니다. 30건은 탐색 기준이며 수익성 입증이나 자동 채택 기준이 아닙니다.</p>
