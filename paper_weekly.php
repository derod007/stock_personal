...
?>
<form class="paper-filter"><label>계좌 <input name="account" value="<?= wh($id) ?>"></label><label>주차 <input type="week" name="week" value="<?= wh($week) ?>"></label><select name="mode"><option value="forward" <?= $mode==='forward'?'selected':'' ?>>앞으로 기록</option><option value="replay" <?= $mode==='replay'?'selected':'' ?>>과거 재현</option></select><button>조회</button></form>
...
<p class="paper-note">선택 주 실행 <?= wh($o['total']) ?>건 · <?= wh(PaperJournal::encode($o['counts'])) ?><br>현재까지 마지막 실행 시작 <?= wh(wt($o['latest_start'])) ?> KST · 읽기 불가 로그 <?= wh($o['invalid_files']) ?>개 (전체 로그 기준)</p><p class="paper-note">로그 0건은 추천 0건과 다릅니다. 예약 시각·휴일 정보를 모르므로 미실행 횟수를 추정하지 않습니다.</p>
<?php endif ?>
<?php if(!$s): ?><p class="paper-empty">계좌 기록이 없어 거래 성과를 집계할 수 없습니다.</p><?php else: ?>
<p class="paper-lede"><?= $s['week_complete']?'달력상 종료된 주':'진행 중인 주 · 부분 집계' ?> · 통화 <?= wh($s['currency']) ?> · 평가 세션 <?= wh($s['evaluation_sessions']) ?>개</p>
<p class="paper-note">현재 계좌 <?= $s['current_account']['halted']?'중단: '.wh($s['current_account']['halt_reason']):'중단 표시 없음' ?> · 현재 마지막 평가 <?= wh(wt($s['current_account']['last_session'])) ?> KST</p>
<?php if($s['account_alerts']): ?><p class="paper-alert">선택 주 실제 발견 중단 이벤트: <?= wh(PaperJournal::encode($s['account_alerts'])) ?></p><?php endif ?>
</section>
<section class="panel">
...
<p class="paper-lede">청산 순손익 <?= wh(wn($s['realized_pnl'])) ?> · 평가자산 변화 <?= wh(wn($s['equity_change'])) ?> <?= wh($s['currency']) ?></p>
...
<?php foreach($r['comparisons_current'] as $c): ?><p class="paper-lede"><?= wh($c['definition']['id']) ?> · <?= wh($c['identity_check']) ?> · <?= $c['source_sync']?'원본과 갱신 일치':'원본과 갱신 불일치' ?><br><?= wh($c['decision'].' / '.implode(', ',$c['reasons'])) ?> · 마지막 평가 <?= wh(wt($c['last_session'])) ?> KST</p><?php if($c['operations']): ?><p class="paper-note">선택 주 비교 래퍼 실행: <?= wh(PaperJournal::encode($c['operations']['counts'])) ?> · 마지막 시작 <?= wh(wt($c['operations']['latest_start'])) ?></p><?php endif; endforeach ?>
